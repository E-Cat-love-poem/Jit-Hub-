  ...
  methods: {
    ...
    async submitCourse() {
          ...
          try {
            ...
            // ✅ 使用改造后的 post 函数调用云函数
            const result = await post('/product/add', formData)
            
            console.log('提交课程响应:', result)
            
            // ✅ 检查云函数返回的真实结果
            console.log('=== 响应验证 ===', result)
            console.log('是否有_id:', !!result._id)
            console.log('是否有id:', !!result.id)
            
            // ✅ 云函数成功标志：返回的数据对象包含 _id
            if (result && (result.code === 200 || result._id)) {
              ...
              // 显示具体错误
              throw new Error(result?.message || `上传失败: ${JSON.stringify(result)}`)
            }
