    ...
    <view v-else>
      ...
      <view class="middle-section">
        ...
        <view class="culture-card">
          ...
          <text class="culture-desc">{{ product.short_desc }}</text>
        </view>
        ...
        <view class="craft-card">
          ...
          <rich-text :nodes="product.detail_desc" class="craft-content"></rich-text>
        </view>
      </view>

      <!-- 底层：操作按钮 -->
      <view class="bottom-section">
        <button class="btn buy" @click="createOrder">确认学习此课程</button>
      </view>
    </view>
  </view>
</template>
...
export default {
  setup() {
   ...
   const loadProductDetail = async () => {
     ...
     try {
       // 1. 修改请求方式，使用查询参数
       const res = await get('/product/detail', { id: productId.value })
    ...
    const createOrder = async () => {
      ...
      try {
        ...
        uni.hideLoading()
        uni.showToast({ 
          title: error.data?.message || '创建课程失败', 
          icon: 'none' 
        })
      }
    }
