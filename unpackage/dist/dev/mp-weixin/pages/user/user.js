...
const _sfc_main = {
  setup() {
    ...
    const loadUserInfo = () => {
      common_vendor.index.__f__("log", "at pages/user/user.vue:72", "=== 从本地缓存加载用户信息 ===");
      const cachedUserInfo = common_vendor.index.getStorageSync("userInfo") || {};
      common_vendor.index.__f__("log", "at pages/user/user.vue:77", "缓存数据:", cachedUserInfo);
      if (cachedUserInfo.userId) {
        ...
        common_vendor.index.__f__("log", "at pages/user/user.vue:88", "用户页面显示:", userInfo.value);
      } else {
        userInfo.value = {
          name: "未登录用户",
          userId: "--",
    ...
    const navigateToEditProfile = () => {
      common_vendor.index.navigateTo({
        ...
        success: () => common_vendor.index.__f__("log", "at pages/user/user.vue:104", "跳转到关于页面")
      });
    };
    const navigateToOrder = () => {
      common_vendor.index.navigateTo({
        url: "/pages/purchased/purchased",
        success: () => common_vendor.index.__f__("log", "at pages/user/user.vue:112", "跳转到课程页"),
        fail: (err) => {
          common_vendor.index.__f__("log", "at pages/user/user.vue:114", "跳转失败:", err);
    ...
    const navigateToAddress = () => {
      common_vendor.index.switchTab({
        url: "/pages/Home/Home",
        success: () => common_vendor.index.__f__("log", "at pages/user/user.vue:127", "跳转到主页"),
        fail: (err) => {
          common_vendor.index.__f__("log", "at pages/user/user.vue:129", "跳转失败:", err);
          common_vendor.index.showToast({
            title: "跳转失败",
            icon: "none"
          });
    ...
    const navigateToSettings = () => {
      common_vendor.index.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            common_vendor.index.__f__("log", "at pages/user/user.vue:147", "用户点击了我知道了");
          }
        }
      });
    };
        ...
        7: () => {
          ...
          showModal("好了？开心了？给我滚去重新登录！", "", false, () => {
            common_vendor.index.navigateTo({
              ...
              fail: (err) => {
                common_vendor.index.__f__("log", "at pages/user/user.vue:168", "跳转到404失败:", err);
              }
            });
          });
        }
