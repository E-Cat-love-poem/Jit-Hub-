...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    common_vendor.onLoad(() => {
      common_vendor.index.__f__("log", "at pages/Home/Home.vue:192", "Home页面加载");
      loadFeaturedProducts();
    });
    common_vendor.onShow(() => {
      common_vendor.index.__f__("log", "at pages/Home/Home.vue:196", "Home页面显示，刷新数据");
      loadFeaturedProducts();
    });
    const loadFeaturedProducts = async () => {
      ...
      try {
        ...
        featuredCrafts.value = res.map((item, index) => {
          let imageUrl = item.imageUrl || item.image_url || "";
          common_vendor.index.__f__("log", "at pages/Home/Home.vue:212", `商品 ${index} 原始图片路径:`, imageUrl);
          if (imageUrl && !imageUrl.startsWith("http") && !imageUrl.startsWith("/")) {
            imageUrl = `/static/images/${imageUrl}`;
            common_vendor.index.__f__("log", "at pages/Home/Home.vue:217", `商品 ${index} 转换后路径:`, imageUrl);
          }
          if (!imageUrl || imageUrl === "null" || imageUrl === "undefined") {
            imageUrl = "/static/images/default-product.png";
            common_vendor.index.__f__("log", "at pages/Home/Home.vue:223", `商品 ${index} 使用默认图片`);
          }
          return {
            id: item.id,
            name: item.name,
        ...
        common_vendor.index.__f__("log", "at pages/Home/Home.vue:237", "最终 featuredCrafts:", featuredCrafts.value);
      } catch (err) {
        common_vendor.index.__f__("error", "at pages/Home/Home.vue:240", "加载失败:", err);
      } finally {
    ...
    const onSearch = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/Home/Home.vue:269", "搜索结果:", products);
        ...
        common_vendor.index.__f__("error", "at pages/Home/Home.vue:304", "搜索失败:", err);
        common_vendor.index.showToast({
          title: "搜索失败",
          icon: "none"
        });
    ...
    const showErrorModal = (message = "功能正在开发中，敬请期待") => {
      common_vendor.index.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            common_vendor.index.__f__("log", "at pages/Home/Home.vue:437", "用户点击了我知道了");
          }
        }
      });
    };
