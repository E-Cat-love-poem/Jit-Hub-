  ...
  methods: {
    async handleLogin(e) {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/Login/Login.vue:65", "发送登录请求:", loginData);
        const res = await utils_request.post("/user/login", loginData);
        common_vendor.index.__f__("log", "at pages/Login/Login.vue:69", "登录响应:", res);
        if (res.success) {
          common_vendor.index.__f__("log", "at pages/Login/Login.vue:72", "登录成功");
          common_vendor.index.__f__("log", "at pages/Login/Login.vue:73", "用户ID:", (_a = res.userInfo) == null ? void 0 : _a.user_id);
          ...
          common_vendor.index.setStorageSync("userInfo", {
            ...
            name: (_e = res.userInfo) == null ? void 0 : _e.user_name,
            // 直接存储名字
            level: "Lv.3",
            levelProgress: 65
          });
          ...
          setTimeout(() => {
            common_vendor.index.switchTab({
              url: "/pages/Home/Home",
              success: () => {
                common_vendor.index.__f__("log", "at pages/Login/Login.vue:100", "跳转到首页成功");
              },
              fail: (err) => {
                common_vendor.index.__f__("log", "at pages/Login/Login.vue:103", "跳转失败:", err);
        ...
        common_vendor.index.__f__("error", "at pages/Login/Login.vue:115", "登录请求异常:", err);
        common_vendor.index.showToast({
          title: "网络错误，请稍后重试",
          icon: "none"
        });
    ...
    handleForgetPassword() {
      common_vendor.index.__f__("log", "at pages/Login/Login.vue:126", "点击了忘记密码");
      common_vendor.index.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            common_vendor.index.__f__("log", "at pages/Login/Login.vue:134", "用户点击了我知道了");
          }
        }
      });
    }
